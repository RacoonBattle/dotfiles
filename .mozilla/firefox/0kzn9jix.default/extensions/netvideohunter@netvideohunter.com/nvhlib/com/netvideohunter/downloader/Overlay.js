...
var Overlay=function (main,window,document) {
	...
	var onEnabledChanged=function(event){
		window.document.getElementById("netvideohunter-button").setAttribute("disabled",!event.value);
		window.document.getElementById("netvideohunter-toolbarbutton").setAttribute("disabled",!event.value);
	}
	...
	this.iconAnim=new function(){
		...
		this.start=function (){
			...
			interval=window.setInterval(update,300);
	...
	this.openMediaListWindow=function(){	
		if(!main.pref.getBoolPref("extensions.netvideohunter.openInTab")){
			...
			"dialog=no,width=820,height=450,centerscreen",this.main,this.window);
	...
	this.updateIconsAppearance=function(){
		...
		try{
			...
			main.pref.setBoolPref("extensions.netvideohunter.icon.statusbar",true);
			statusbarpanel.setAttribute("hidden","true");
		}
	...
	this.onLoad=function(e) { 		
		...
		if(statusbarButton) statusbarButton.setAttribute("disabled",!main.enabled);
		var toolbarButton=window.document.getElementById("netvideohunter-toolbarbutton");
		if(toolbarButton) toolbarButton.setAttribute("disabled",!main.enabled);
	...
	this.onIgnoreClick=function(event){
		...
		if(currentSite){
			if(main.ignoreSites.getItemById(currentSite)===null){
				main.ignoreSites.addItem({id:currentSite});
			}else{
	...
	this.getCurrentSite=function(){
		...
		return UriUtils.mainHostFromUrl(url);
	}
