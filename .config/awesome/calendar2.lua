...
function switchNaughtyMonth(switchMonths)
        ...
        calendar[3].box.widgets[2].text = string.format('<span font_desc="%s">%s</span>', "monospace 12", displayMonth(calendar[1], calendar[2], 2))
end
...
function addCalendarToWidget(mywidget, custom_current_day_format)
  ...
  mywidget:add_signal('mouse::enter', function ()
        ...
        calendar = { month, year,
        naughty.notify({
                text = string.format('<span font_desc="%s">%s</span>', "monospace 12", displayMonth(month, year, 2)),
                timeout = 0,
                hover_timeout = 0.5,
